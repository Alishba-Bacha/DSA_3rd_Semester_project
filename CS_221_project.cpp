...
int HashCity(const char* cityName) 
{
    ...
    {
        hash = (hash * 31 + cityName[i]) % MAXCITY;
    }
    return hash;
}
...
void MakeReservation() 
{
    ...
    else 
    {
        ...
        cout << "Error: Route not found. Reservation cannot be made." << endl;
    }
}
...
void ReadFlightData() {
    ...
    MakeFlightNode(111, "Islamabad", 700, "Karachi", 830);
    
    flightList[1].FlightNo = 222;
    MakeFlightNode(222, "islamabad", 800, "New York", 900);
    
    // Initialize cityList entries for departure cities
    int indexA = HashCity("Islamabad");
    cityList[indexA].cityName = strdup("Islamabad");
    cityList[indexA].nextDeparture = flightList[0].flight;

    // Initialize cityList entries for arrival cities
    int indexB = HashCity("Karachi");
    cityList[indexB].cityName = strdup("Karachi");
    cityList[indexB].nextArrival = flightList[0].flight;

    int indexC = HashCity("New York");
    if (cityList[indexC].cityName == nullptr) {
        cityList[indexC].cityName = strdup("New york");
    }
    cityList[indexC].nextArrival = flightList[1].flight;

    // Link flights in the departure list
...
void DisplayFlightInfo(FlightType *flight) {
    if (flight != nullptr) {
        cout << flight->FlightNo << " " << flight->startCity << " " << flight->timeDepart 
             << " " << flight->endCity << " " << flight->timeArrival << endl;
    }
}

void DisplayAllFlightsData() 
{
    cout << "FlightNo\tDeparture City\tDeparture Time\tArrival City\tArrival Time" << endl;
    cout << "-------------------------------------------------------------" << endl;
...
{
    cout << "Cities Serviced by Airline:" << endl;
    cout << "----------------------------" << endl;
    for (int i = 0; i < MAXCITY; ++i) 
    {
...
void PrintPassengers(int FlightNo) {
    ReservationType *current = reserveHead;
    cout << "Passengers on Flight " << FlightNo << ":" << endl;
    while (current != nullptr) {
